...
sol_idx = 10#18 
pattern_tf = np.array([0,0, np.pi/180 *60])                   # [x_translation (m), y_translation (m), rotation (rad)]
...
for j, cart_hex_centers in enumerate(centers):
    ...
    for i, pattern in enumerate(cart_hex_centers):
        ...
        ax.add_artist(cen)
        ax.set(xlim=(-prim_diam*1.1/2, prim_diam*1.1/2), ylim=(-prim_diam*1.1/2, prim_diam*1.1/2))

        plt.title(titles[j])
        plt.xlabel(xlabels[j])
        plt.xticks([])
        plt.yticks([])  
        plt.legend(handles = lgnd_patches, labels = ["Pattern 1", "Pattern 2", "Pattern 3", "Pattern 4"],
                   loc = "upper right" )
...
def get_splodges_mask(im_array, min_peak, radius, prox_thresh = 1):
    ...
    """
    splodge_coords = feature.peak_local_max(onp.asarray(im_array), threshold_abs=min_peak, min_distance=prox_thresh) 
...
dummy_jewel_mask = JewelMask(mask_pattern=dummy_mask_pattern, 
                       n_pix=n_pix, 
                       pixel_coords=pixel_coords, 
                       wedge_angles=np.array([wedge_angle, wedge_angle]),
                       slope_orientations= np.array([np.pi/180 *45 , np.pi/180 *135]), 
...
dummy_psf.set_windowing_parameters(sub_psf_window_sz= sub_psf_window_sz,  
                                  sigma= 15,           
                                  neighborhood_size= 10,   
                                  thresh_diff= 0.1e-11,         
                                  prox_thresh= 10,              
...
# For one NR pattern
single_pat_cens = mask_pattern.cart_hex_centers[1] # just grabbing first (no justification really)
...
JM_dI = float(jwl_psf.sum()/init_intensity * 100)
OM_dI = float(op_psf.sum()/init_intensity * 100)

print("Percentage transmission of incoming light:")
print("Jewel mask: %{} \nOpaque mask %{}".format(round(JM_dI,2),round(OM_dI,2)))
print("Jewel mask transmits {} times greater total intensity than opaque mask".format(round(float(jwl_psf.sum()/op_psf.sum()), 2)))
